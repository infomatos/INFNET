...
export default function OfertasScreen({ navigation }) {
  ...
  const handleContribuir = async () => {
    ...
    try {
      ...
      Alert.alert(
        "Obrigado!",
        `Sua ${tipo.toLowerCase()} de ${formatBRL(v)} foi registrada. Deus abençoe!`
      );
    } catch (e) {
  ...
  return (
    <ScrollView style={{ backgroundColor: "#fff" }} contentContainerStyle={styles.wrap}>
      {/* HERO */}
      <View style={styles.hero}>
        <Image
          source={{ uri: "https://images.unsplash.com/photo-1500382017468-9049fed747ef?q=80&w=1800&auto=format&fit=crop" }}
          style={styles.heroImg}
        />
        <Text style={styles.verso}>
          “Honra ao Senhor com os teus bens e com as primícias de toda a tua renda.”{" "}
          <Text style={styles.versoRef}>Provérbios 3:9</Text>
        </Text>
      </View>
      ...
      <View style={[styles.cardConta, { borderColor: "#EC7000" }]}>
        ...
        <Text style={styles.contaLinha}>Agência: <Text style={styles.bold}>1234</Text></Text>
        <Text style={styles.contaLinha}>Conta: <Text style={styles.bold}>56789-0</Text></Text>
        <Text style={styles.contaLinha}>Titular: <Text style={styles.bold}>Associação GUIA Church</Text></Text>
        <Text style={styles.contaPix}>PIX (CNPJ): <Text style={styles.bold}>12.345.678/0001-90</Text></Text>

        <View style={styles.contaActions}>
          <Pressable style={[styles.btnGhost, { borderColor: "#EC7000" }]} onPress={() => copy("12.345.678/0001-90")}>
            <Text style={[styles.btnGhostTxt, { color: "#EC7000" }]}>Copiar Pix</Text>
          </Pressable>
        </View>
      </View>
      ...
      <View style={[styles.cardConta, { borderColor: "#21C25E" }]}>
        ...
        <Text style={styles.contaLinha}>Usuário: <Text style={styles.bold}>@guiachurch</Text></Text>
        <Text style={styles.contaPix}>Chave Pix: <Text style={styles.bold}>ofertas@guiachurch.com</Text></Text>

        <View style={styles.contaActions}>
          <Pressable style={[styles.btnGhost, { borderColor: "#21C25E" }]} onPress={() => copy("ofertas@guiachurch.com")}>
            <Text style={[styles.btnGhostTxt, { color: "#21C25E" }]}>Copiar chave</Text>
          </Pressable>
        </View>
      </View>
      ...
      <Modal visible={openModal} transparent animationType="fade" onRequestClose={() => setOpenModal(false)}>
        ...
        <View style={styles.modal}>
          ...
          <View style={styles.tipoRow}>
            <Chip selected={tipo === "Dizimo"} onPress={() => setTipo("Dizimo")} label="Dízimo" />
            <Chip selected={tipo === "Oferta"} onPress={() => setTipo("Oferta")} label="Oferta" />
          </View>
          ...
          <View style={styles.modalActions}>
            <Pressable style={styles.btnGhost} onPress={() => setOpenModal(false)}>
...
function Chip({ selected, onPress, label }) {
  return (
    ...
    >
      <Text style={[styles.chipTxt, selected ? { color: "#fff" } : null]}>{label}</Text>
    </Pressable>
  );
}
...
const styles = StyleSheet.create({
  ...
  hero: { borderBottomWidth: 1, borderColor: "rgba(0,0,0,0.06)" },
  heroImg: { width: "100%", height: 160, backgroundColor: "#eee" },
  verso: { padding: 16, paddingTop: 12, color: "#111827", lineHeight: 20, fontSize: 14 },
  versoRef: { fontWeight: "800" },

  sectionTitle: { color: "#111827", fontWeight: "800", fontSize: 18, marginTop: 16, marginBottom: 10, paddingHorizontal: 16 },
  ...
  bankHeader: { flexDirection: "row", alignItems: "center", marginBottom: 10, gap: 8 },
  bankBadge: { paddingHorizontal: 10, paddingVertical: 6, borderRadius: 10 },
  bankBadgeTxt: { color: "#fff", fontWeight: "900", letterSpacing: 0.3, textTransform: "lowercase" },
  ...
  modalTitle: { color: "#111827", fontWeight: "800", fontSize: 18, marginBottom: 12 },
  modalLabel: { color: "#111827", fontWeight: "700", marginTop: 8, marginBottom: 6 },
  ...
  input: {
    ...
    borderRadius: 12, paddingHorizontal: 12, paddingVertical: 10, color: "#111827",
  },
