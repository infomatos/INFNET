...
export default function CursoForm({ visible, inicial, onClose, onSave }) {
  ...
  useEffect(() => {
    setV(inicial || { status: 'Ativo', progresso: 0 });
  }, [inicial]);
  ...
  async function salvar() {
    ...
    await onSave({
      ...v,
      carga_horaria: v.carga_horaria ? Number(v.carga_horaria) : 0,
      progresso: v.progresso ? Number(v.progresso) : 0,
    });
    setSalvando(false);
  }
    ...
    <Modal
      ...
      <View style={styles.backdrop}>
        <View
          style={styles.card}
          accessible
          accessibilityLabel="Formulário de curso">
          <Text style={styles.titulo}>
            {v.id ? 'Editar curso' : 'Novo curso'}
          </Text>
          ...
          {[
            {
              k: 'titulo',
              label: 'Título',
              placeholder: 'Ex.: Discipulado 101',
            },
            {
              k: 'descricao',
              label: 'Descrição',
            ...
            {
              k: 'progresso',
              label: 'Progresso (0-100%)',
              placeholder: '0',
              props: { keyboardType: 'numeric', maxLength: 3 },
            },
          ].map((f) => (
          ...
          <View style={styles.row}>
            <Pressable
              onPress={() =>
                set('status', v.status === 'Ativo' ? 'Inativo' : 'Ativo')
              }
              style={styles.badge}>
          ...
          <View style={styles.acoes}>
            <Botao
              ...
              onPress={salvar}
              style={{ flex: 1 }}
              icone={salvando ? <ActivityIndicator /> : null}
            />
            <Botao
              titulo="Cancelar"
              variante="secundario"
